...
def register_node():
    ...
    if nodes is None:
        return jsonify({"error": "Proszę podać poprawną listę wężłow"}), 400

    for node in nodes:
        blockchain.jj_register_node(node)

    response = {
            'message': 'Dodano nowy węzęł',
...
def consensus():
    replaced = blockchain.jj_resolve_conflicts()

    if replaced:
        response = {
                'message': 'Nasz łańcuch zostal zastąpiony',
                'new_chain': blockchain.jj_chain
        }
    else:
        response = {
                'message': 'Nasz łańcuch jest autorytatywny (aktualny)',
                'chain': blockchain.jj_chain
        }
